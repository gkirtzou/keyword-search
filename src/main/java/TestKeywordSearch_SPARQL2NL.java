...
import org.aksw.sparql2nl.TEST;
import org.aksw.sparql2nl.Sparql2nl;

/**
 *
 * @author gkirtzou
 */
public class TestKeywordSearch_SPARQL2NL {
    ...
    public static void main(String[] args) throws Exception { 
        ...
        key2sparql.getSparqlFromKeywords(keywordInput, init.getDB(), 
                         endpoint, prefixes, query_prefix);
        ArrayList<SPARQL> sparqlQueryList = key2sparql.getQueryList();
        String keywordsMessage = key2sparql.getKeywordsMessage();
       // TEST nl = new TEST();
        Sparql2nl nl = new Sparql2nl();
        nl.init_url();
        ...
        for(SPARQL q : sparqlQueryList) {
          ...
          //  System.out.println(i + ":" + query);
         /*   query = PREFIX + query;
            String nl_query[] = nl.getQueryNL(query, URL);  */
            String nl_query[] = nl.getQueryNL(query); /* */
            System.out.println("########## ########## ########## ##########");
            System.out.println(i + ": Before postprocessing:");
            System.out.println(nl_query[0]);
            System.out.println(i + ": After postprocessing:");
            System.out.println(nl_query[1]);
            System.out.println("########## ########## ########## ##########");
        }
       ...
       // System.out.println(init.getConstants().endpoint);
        ...
        if(sparqlQueryList.size() != 0) {
            // Select query
            int queryId = 0;
            System.out.println("Selected SPARQL query:: " + queryId + " (we select the first-one..for now..)");

            // Prepare query for execution
            String query = sparqlQueryList.get(queryId).getSparqlQuery();
            query += " LIMIT 100";
