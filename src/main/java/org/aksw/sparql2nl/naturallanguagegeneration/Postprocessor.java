...
//                 m_p_var_s = Pattern.compile(p_var_s).matcher(b);
    ...
    private String getVerb(NLGElement el) {
      ...
      //  try{
        if (vp != null) {
            //VPPhraseSpec vpps = 
            WordElement we = new WordElement(vp.getFeatureAsString("head"));// EDW SKAEI !!!! <<---------- ------ ----
            return we.getBaseForm();
//            return (String)((WordElement) vp.getFeature("head")).getBaseForm();
        }//}catch (Exception ea)
        //{System.out.println();
        //System.err.println("####### " + ea +" ##### Edw getVerb() ths Postprocessor: SKAW!!! PETAW EXCEPTION !!!! #####");}
        return null;
    }
            ...
            for (Filter f : filters) {
                ...
                for (Sentence s : f.sentences) {
                    ...
                    if (fstring.startsWith(var + " matches ")) {
                        ...
                        WordElement we = new WordElement(sentence.sps.getVerb().toString());
                        if (we.getBaseForm().equals("be")
        ...
        for (NPPhraseSpec sel : selects) {
            ...
            if (oc == 0) sel.setFeature("head","");
                ...
                for (Sentence sentence : sentences) {
                    ...
                    WordElement we = new WordElement(s.getVerb().toString());
                    if (we.getBaseForm().equals("be")
